...
> ✅ **`F-1` / `F-2` fixed** (2026-08-14), **`F-3` fixed** (2026-08-17), and the mis-parsed rows have since been replaced — see [`G-2`](#follow-ups-from-f-3).
...
| **A-8** ✅ | **Nothing in Max can create a period except an import.** Periods only exist because a spreadsheet was uploaded. The moment the founder stops keeping the sheet — which is the entire V1 bar — there is no way for next month to begin. Related: he aligns periods loosely to a card cycle ending the 8th, deliberately imprecise, 4–5 weeks with a few days' slack. **Closed 2026-09-01:** three routes now create a period without an import — `startFirstPeriod` for an empty account, the rollover prompt when a month ends, and `/start-month` for a month that has not been opened yet. All three are a button press, never a page-load write. | ~~Before V1 can actually be used for a second period.~~ This is a hole in "replaces my spreadsheet", not a nice-to-have. | Max becomes a viewer for spreadsheets he has stopped making. Options: roll a new period automatically from the observed cycle, offer it and let him confirm, or ask once at setup. Inferring from the periods already imported is preferred over asking — the start dates cluster, and a question at setup is the labelling tax the vision argues against ([`A-5`](#a--decisions-only-the-founder-can-make)). |
| **A-9** | **Financial data leaving the box.** The screenshot reader sends an image of the user's own transactions to a third-party model provider. The red lines cover dark patterns, moralising, ranking and memory scope — none of them say anything about what may be sent to an outside service, or on whose behalf. Deferred deliberately: the founder is the only account, and no invitations have been sent. | **Before a second person is given the app**, and before any LLM feature runs over stored data rather than a one-off upload. | Sending someone else's financial data to a third party without a stated position is the kind of thing this product cannot be caught doing once. It is cheap to write the rule now and awkward to retrofit an apology. Note this is a *different* question from A-5's setup burden — that one is about effort, this one is about trust. |
---
...
| **G-2** ✅ | ~~**Re-upload the real file.**~~ **Closed 2026-09-01.** Written on the day `F-3` was fixed, describing the database as it stood at that moment, and never revisited across two weeks that included a data-model migration (`0004`) and the whole V1 build. The founder checks the live figures against his spreadsheet as a matter of habit and reports August 2026 agreeing, which is direct current evidence against a stale note. **Scope of that check:** August only. A period imported before 2026-08-17 and never re-imported would still carry the `F-3` signature — income of £2,285 (the rent) and bills of about £416 across 5 rows — which is specific enough to spot by eye. [`G-3`](#follow-ups-from-f-3) would make the app answer this for itself instead of anyone having to remember. |
